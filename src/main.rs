...
fn main() {
    ...
    let shooting_sound = audio
        .new_sound("assets/shoot.wav")
        .expect("shooting audio didn t load");
    let bg_music = audio
        .new_music("assets/bgmusic.wav")
        .expect("couldnt load bg music");
    ...
    while !rl.window_should_close() {
        ...
        for asteroid in asteroids.iter_mut() {
            if aabb_collision(
                player.pos,
                Vector2::new(
                    player.sprite.width as f32 / 2.0,
                    player.sprite.height as f32 / 2.0,
                ),
                asteroid.pos,
                Vector2::new(
                    asteroid.sprite.width as f32 / 2.0,
                    asteroid.sprite.height as f32 / 2.0,
                ),
            ) {
        ...
        asteroids.retain(|asteroid| {
            ...
            for bullet in bullets.iter_mut() {
                if aabb_collision(
                    ...
                    bullet.pos,
                    Vector2::new(
                        bullet.sprite.width as f32 / 2.0,
                        bullet.sprite.height as f32 / 2.0,
                    ),
                ) {
